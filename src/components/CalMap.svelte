<script>
  ...
  async function loadSpecificData() {
    ...
    specific_data = response.flatMap((d) =>
      columns.map((year) => ({
        County: d.County,
  ...
  function updateScaleBar(svg, maxCrimeCount) {
    ...
    svg.select(".scale-bar text:first-of-type").text(minValue);

    svg.select(".scale-bar text:last-of-type").text(maxCrimeCount);
  }
  ...
  onMount(() => {
    ...
    svg
      ...
      .on("mouseover", function (event, d) {
        const countyData = specific_data.filter(
          (c) => c.County === d.properties.name
        );
        const selectedYear =
          document.getElementById("selectedYear").textContent;
        const crimeCount = countyData.filter(
          (c) => c.Year === parseInt(selectedYear, 10)
        );
        const violent = crimeCount.filter(
          (c) => c.Crime_Category === "Violent Crimes"
        );
        const property = crimeCount.filter(
          (c) => c.Crime_Category === "Property Crimes"
        );
        const arson = crimeCount.filter((c) => c.Crime_Category === "Arson");
        const countyTotalData = data.filter(
          (c) => c.County === d.properties.name
        );
        const totalCrimeCount = countyTotalData.filter(
          (c) => c.Year === parseInt(selectedYear, 10)
        );
    ...
    function addScaleBar() {
      ...
      const scaleBar = svg
        ...
        .attr(
          "transform",
          `translate(${width - scaleBarWidth - 50}, ${height - 500})`
        );
    ...
    function highlightCounty() {
      svg.selectAll(".subunit").style("fill", (d) => {
        return d.properties.fullName === selectedCounty
          ? "orange"
          : "steelblue";
      });
    }
    ...
    function updateHeatMap(year) {
      ...
      svg.selectAll(".subunit").style("fill", (d) => {
        ...
        const fillColor = countyData
          ? colorScale(countyData.Crime_Count)
          : "gray";
        return fillColor;
      });
    ...
    document
      .getElementById("countyDropdown")
      .addEventListener("change", function (event) {
        selectedCounty = event.target.value;
        highlightCounty();
        // selectedCounty = d.properties.fullName;
        const countyData = specific_data.filter(
          (c) => c.County === selectedCounty.replace(" County", "")
        );
        const selectedYear =
          document.getElementById("selectedYear").textContent;
        const crimeCount = countyData.filter(
    ...
    const countiesData = feature(cal, cal.objects.subunits).features.map(
      (d) => d.properties.fullName
    );
    const dropdown = document.getElementById("countyDropdown");
    countiesData.sort();
